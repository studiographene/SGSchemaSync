...
export async function generateFilesForTag(
  ...
  for (const opInfo of operations) {
    ...
    const baseName = operationId ? toPascalCase(operationId) : getPathBasedBaseName(path);
    const functionName = toTsIdentifier(operationId || `${method}_${path.replace(/[^a-zA-Z0-9_]/g, "_")}`);

    // Add Operation Banners to Types File
    const summary = operation.summary || "No Description Provided";
    const opDescBanner = createBanner(summary);
    const opRouteBanner = createRouteBanner(method, path);
    typesContent += `\n\n${opDescBanner}\n${opRouteBanner}\n`; // Add banners before types for this op
    ...
    const methodUpper = method.toUpperCase(); // Uppercase method for naming

    // --- Generate Request Body Type --- (and store name)
    if (operation.requestBody && "content" in operation.requestBody) {
      const requestBodySchema = operation.requestBody.content?.["application/json"]?.schema;
      if (requestBodySchema) {
        const typeName = `${baseName}_${methodUpper}_Request`;
      ...
      if (statusCode.startsWith("2")) {
        ...
        if (isPrimary) {
          if (response && "content" in response && response.content?.["application/json"]?.schema) {
            ...
            const typeName = `${baseName}_${methodUpper}_Response${isPrimary ? "" : statusCode}`;
          ...
          if (response && "content" in response && response.content?.["application/json"]?.schema) {
            const responseSchema = response.content["application/json"].schema;
            const typeName = `${baseName}_${methodUpper}_Response${statusCode}`; // Keep status code for non-primary
            if (!generatedTypeNames.has(typeName)) {
              try {
                const tsType = await compile(responseSchema as JSONSchema, typeName, { bannerComment: "" });
                typesContent += `\n${tsType}\n`;
    ...
    if (queryParams && queryParams.length > 0) {
      ...
      if (Object.keys(paramsSchema.properties || {}).length > 0) {
        const typeName = `${baseName}_${methodUpper}_Parameters`;
