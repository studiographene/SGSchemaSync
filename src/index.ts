...
export async function runGenerator(options: GeneratorOptions): Promise<void> {
  ...
  try {
    ...
    for (const tagName in operationsByTag) {
      ...
      if (packageConfig.useDefaultRequester) {
        clientModuleContent += `import type { SGSyncRequester } from '@studiographene/sg-schema-sync/requester-types'; // Adjust if pkg structure differs\n`;
      }
      ...
      if (packageConfig.useDefaultRequester) {
        ...
        clientModuleContent += `import { createDefaultSGSyncRequester } from \'@studiographene/sg-schema-sync/default-requester\'; // Adjust path if needed\\n\\n`;
...
function generateCustomRequesterScaffold(exportName: string, packageConfig: ResolvedPackageConfig): string {
  ...
  content += `import type { SGSyncRequester, SGSyncRequesterOptions, SGSyncResponse } from '@studiographene/sg-schema-sync/requester-types'; // Adjust path if necessary\\n\\n`;
