...
// Regex for the top-level banner, matching its specific multi-line structure.
const generatedBannerRegex = new RegExp(
  "^\\/\\*\\*+\\n" + // Line 1: /* followed by one or more * then newline
    "\\s*\\*\\s.*?Generated by sg-schema-sync.*?\\*\\n" + // Line 2: optional leading ws, *, space, content with timestamp, *, newline
    "\\s*\\*\\s.*?DO NOT EDIT THIS FILE MANUALLY.*?\\*\\n" + // Line 3: optional leading ws, *, space, warning, *, newline
    "\\s\\*+\\/(\\n)?", // Line 4: space, one or more *, */, optional final newline. (No longer has \*\/ before (\n)?)
  "m"
);
